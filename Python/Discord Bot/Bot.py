...
#    members = "\n - ".join([member.name+" - "+str(member.roles) for member in guild.members])
    ...
    if message.channel.name == "public-announcements":
        ...
        if payload.emoji.name == '☑️' or payload.emoji.name == '✅': #checkboxes 2611 2705
            ...
            for user in guild.members:
                user.remove_roles(DM, hunt, fugi, play, reason="Game start")
    ...
    if message.content[0] == '/':
        ...
        if content[0] == "/start":
            for role in message.author.roles:
                if role.id == 784287952012050452: break #Playtesters
        ...
        if content[0] == "/setloc":
            for role in message.author.roles:
                if role.id == 784287952012050452: break #Playtesters
                if role.id == 783900332757352458: break #DM/news boi
                if role.id == 784262916844683285: break #Tech Support
            else:
                await message.channel.send("You don't have permission to send this command.")
                return
